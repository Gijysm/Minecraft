...
void LightSolver::Add(int x, int y, int z, int emmision)
{
	...
	chunk->modifier = true;
	chunk->lightmap->Set(entry.x - chunk->x * _CHUNK_W, entry.y - chunk->y * _CHUNK_H, entry.z - chunk->z * _CHUNK_D,channel, entry.light);
}
...
void LightSolver::remove(int x, int y, int z)
{
	...
	chunk->lightmap->Set(x - chunk->x * _CHUNK_W, y - chunk->y * _CHUNK_H, z - chunk->z * _CHUNK_D, channel, 0);
	...
	while (!addQueue.empty())
	{
		...
		for (size_t i = 0; i < 6; i++)
		{
			...
			if (chunk)
			{
				...
				{
					chunk->lightmap->Set(entry.x - chunk->x * _CHUNK_W, entry.y - chunk->y * _CHUNK_H, entry.z - chunk->z * _CHUNK_D, channel, entry.light - 1);
					chunk->modifier = true;
					Lightentry nentry;
					nentry.x = x;
					nentry.y = y;
